...
const[progressArray, setProgressArray]= useState([
  ...
  {lectureCompleted:2, totalLectures: 11},
  ...
  return (
    ...
    <div className='md:px-36 px-8 pt-10'>
          <h1 className='text-2xl font-semibold'>My Enrollments 1</h1>
            ...
            <tbody className=' text-gray-700'>
              {enrolledCourses.map((course, index)=>(
                <tr key={index} className='border-b border-gray-500/20'>
                  ...
                  <td className='px-4 py-3 max-sm:hidde'>{progressArray[index] && `${progressArray[index].lectureCompleted}/ 
                  ${progressArray[index].totalLectures}`} <span>Lectures</span></td>

                  <td className='px-4 py-3 max-sm:text-right'> 
                    <button onClick={()=> navigate('/player/'+course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>{ progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Completed' : 'On Going'}</button></td>
